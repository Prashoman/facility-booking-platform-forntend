...
export default function NavBar() {
  ...
  <div className="w-full sticky z-30 top-0 left-0">
    <nav className="bg-gradient-to-r from-blue-800 to-blue-900 text-white w-full px-4 xl:px-20 shadow-2xl">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-2">
        {/* Updated Logo */}
        <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <img className="h-[50px] w-[80%]" src="https://i.ibb.co.com/8gXLkFx2/logo.webp" alt="logo"></img>
        </Link>
        ...
        <div className="flex items-center space-x-3 rtl:space-x-reverse">
          <ul className="hidden md:flex items-center space-x-8 rtl:space-x-reverse">
            ...
            <li>
              <Link
                to="/contact-us"
                className={`text-white hover:text-[#F2BF4A] text-[16px] font-[500] hover:text-[17px] transition-all ${
                  currentPathname === `/contact-us` ? "text-[#F2BF4A]" : ""
                }`}
              >
                Contact Us
              </Link>
          ...
          {user ? (
            <Link
              ...
              className="bg-green-500 text-white px-2 py-1 rounded-md hidden lg:block"
            >
              Dashboard
            </Link>
          ) : (
  ...
  {isNavbarOpen && (
    <motion.div
      className="w-[50%] bg-gradient-to-r from-blue-800 to-blue-900 text-neutral-content h-screen block lg:hidden fixed z-30 top-[15%] right-0 rounded"
